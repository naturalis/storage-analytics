...
project_ids_skip = ['baf09a7e28bf448092707fa9917f8aae','b60f923463e04a4bb1858388aed6239f']
### End Settings
### start API's


keystone = KeyStone(auth_url_no_ssl,ks_username,ks_password,project_name,ca_bundle)
for p in keystone.list_projects():
    if p['id'] in project_ids_skip:
        log.logger.debug('Excluding volumes in project %s with id %s' % (p['name'],p['id']))
    else:
        log.logger.debug('Checking volumes in project %s with id %s' % (p['name'],p['id']))
        cinder = Cinder(auth_url_no_ssl,ks_username,ks_password,project_name,ca_bundle,p['id'])
        nova = Nova(auth_url_no_ssl,ks_username,ks_password,p['id'],ca_bundle)
        ...
        for v_i in volume_info:
            ...
            if not v_i['attached_to_id'] == '':
                v_i['attached_to_name'] = nova.get_server_name_from_id(v_i['attached_to_id'])
            else:
