    ...
    def update_access_to_project(self, project_name, group_names):
        ...
        for g in self.grouplist:
            if g.name in excludes:
                next
            try:
                access = self.ksclient.roles.check(self.member_role_id,
                                                   group=g.id,
                                                   project=project_id)
                current_access.append(g.name)
            except:
                log.logger.debug("Assuming that group '%s' has no access to '%s'"
                                 % (g.name, project_name))
